...
* **印刷**: `character-sheet.png` を背景にしたキャラクターシートを印刷。背景が表示されたPDFまたは画面プレビューを使用し、ブラウザの印刷機能で出力可能。
* **PDF生成**: キャラクターシートをデザインされた背景画像付きのPDFファイルとして生成し、自動で印刷ダイアログを表示。
...
このリポジトリにはJestを用いた自動テストが含まれており、Node.js環境で実行できます。
